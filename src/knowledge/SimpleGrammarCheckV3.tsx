...
function sameQuestionOrder(a: readonly GrammarQuestionV3[], b: readonly GrammarQuestionV3[]): boolean {
...
export default function SimpleGrammarCheckV3() {
  ...
  const begin = (nextCategory: GrammarCategoryV3, nextQuestions = nextCategory.questions) => {
    let randomized = shuffleGrammarQuestions(nextQuestions);
    if (category?.id === nextCategory.id && sameQuestionOrder(randomized, questions) && randomized.length > 1) {
      randomized = shuffleGrammarQuestions(randomized);
    }
  ...
  const reshuffleRemaining = () => {
    if (selected !== null || questions.length - index < 2) return;
    setQuestions((current) => shuffleRemainingGrammarQuestions(current, index));
    setSelected(null);
  };
  ...
  return <>{launcher}{open && createPortal(
    <div
      ...
      data-shuffle-version={GRAMMAR_SHUFFLE_VERSION}
      role="dialog"
      aria-modal="true"
      aria-label="文法チェック"
    >
        ...
        <main ref={mainRef} className="flex-1 overflow-y-auto px-4 py-5 sm:py-8">
          <div className="max-w-2xl mx-auto">
                ...
                {GRAMMAR_CATEGORIES_V3.map((item) => {
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block mt-1 text-sm font-bold text-slate-500 dark:text-slate-400">{item.questions.length}問・毎回シャッフル</span>
                    </span>
            ...
            {view === 'quiz' && category && question && (
              <section
                data-ui="grammar-quiz-question"
                data-question-id={question.id}
                data-question-order={questions.map((item) => item.id).join(',')}
              >
                <div className="flex items-center justify-between gap-2 mb-4">
                  <div className="flex items-center gap-2 min-w-0">
                    <button type="button" onClick={() => setView('hub')} className="min-h-10 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center gap-1 text-sm font-bold shrink-0"><ChevronLeft size={18} />3分野</button>
                    <button
                      type="button"
                      data-ui="grammar-shuffle"
                      onClick={reshuffleRemaining}
                      disabled={selected !== null || questions.length - index < 2}
                      aria-label="残りの問題をシャッフル"
                      className="min-h-10 px-3 rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/30 text-violet-700 dark:text-violet-200 flex items-center gap-1.5 text-sm font-black disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
                    >
                      <Shuffle size={17} />シャッフル
                    </button>
                  </div>
            ...
            {view === 'result' && category && (
              <section className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 sm:p-9 text-center shadow-sm" data-ui="grammar-quiz-result">
                ...
                <div className="mt-6 grid gap-3">
                  {sessionMistakes.length > 0 && <button type="button" onClick={() => begin(category, sessionMistakes)} className="min-h-12 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-black flex items-center justify-center gap-2"><RotateCcw size={18} />間違えた{sessionMistakes.length}問だけ（シャッフル）</button>}
                  <button type="button" onClick={() => begin(category)} className="min-h-12 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-black flex items-center justify-center gap-2"><Shuffle size={18} />もう一度（シャッフル）</button>
