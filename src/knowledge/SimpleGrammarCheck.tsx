...
import { Brain, Home, Link, Clock, ListChecks, ChevronRight, ChevronLeft, CheckCircle2, XCircle, RotateCcw } from 'lucide-react';
import { useDialog } from '../workbook/useDialog';
import { GRAMMAR_CATEGORIES, type GrammarCategory, type GrammarQuestion } from './grammar_sets';
...
export default function SimpleGrammarCheck() {
  ...
  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(mistakeIds)); } catch { /* private mode */ }
  }, [mistakeIds]);
  ...
  const begin = (nextCategory: GrammarCategory, nextQuestions = nextCategory.questions) => {
  ...
  const launcher = target ? createPortal(
    ...
    >
      ...
      <span className="mt-1 text-xs text-slate-600 dark:text-slate-300">3分野から選ぶ</span>
    </button>,
    target,
  ) : null;

  return <>{launcher}{open && createPortal(
    <div className="fixed inset-0 z-[120] bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100" data-ui="knowledge-dialog" role="dialog" aria-modal="true" aria-label="文法チェック">
        ...
        <main ref={mainRef} className="flex-1 overflow-y-auto px-4 py-5 sm:py-8">
          <div className="max-w-2xl mx-auto">
            ...
            {view === 'quiz' && category && question && (
              <section data-ui="grammar-quiz-question">
                <div className="flex items-center justify-between gap-3 mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs font-bold text-violet-600 dark:text-violet-300 mt-0.5">{question.level}</div>
                  </div>
                </div>
                ...
                <article className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 sm:p-8 shadow-sm">
                  ...
                  {selected !== null && (
                    <div className={`mt-6 rounded-2xl p-4 ${selected === question.correctIndex ? 'bg-emerald-50 dark:bg-emerald-950/30' : 'bg-rose-50 dark:bg-rose-950/30'}`} role="status">
                      ...
                      <p className="mt-2 leading-relaxed text-sm sm:text-base text-slate-700 dark:text-slate-200">{question.explanation}</p>
