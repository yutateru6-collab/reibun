...
assert.equal(numbers.length, 102); assert.equal(new Set(numbers).size, 102);
...
for (const s of TOPIC_SETS) { assert.ok(s.ids.length >= 1 && s.ids.length <= 6); assert.ok(s.ids.every(id => QUESTIONS.find(q => q.id === id)?.stage === s.stage)); }
...
async function bounds() {
  ...
  assert.ok(value.scroll <= value.client + 1, JSON.stringify(value)); assert.deepEqual(value.bad, [], JSON.stringify(value)); assert.ok(value.footer <= value.height + 1, JSON.stringify(value));
}
async function answerCurrent() {
  const article = dlg().locator('[data-question]'); const n = Number(await article.getAttribute('data-question')); const q = QUESTIONS[n - 1];
  ...
  await article.locator('[data-action="check"]').click(); await article.locator('[data-ui="knowledge-feedback"]').waitFor();
  assert.equal(await article.locator('.tk-explanation-block p').innerText(), q.explanation); assert.ok(await article.locator('.tk-feedback-label').count() >= 2);
...
try {
  ...
  await page.goto(base + '/#knowledge', { waitUntil: 'networkidle' }); await dlg().waitFor();
  assert.equal(await dlg().getAttribute('data-design'), 'knowledge-learning-hub-v2'); assert.equal(await dlg().locator('[data-start]').count(), 17);
  await bounds(); await page.screenshot({ path: `${out}/mobile-home.png` });
  // Existing-version state survives; looking at a guide must not replace a session.
  ...
  await page.reload({ waitUntil: 'networkidle' }); await dlg().locator('[data-guide="1"]').click(); await dlg().locator('[data-ui="knowledge-guide"]').waitFor();
  assert.equal(await dlg().locator('.tk-point').count(), 3);
  assert.deepEqual(await page.evaluate(key => JSON.parse(localStorage.getItem(key)!), STORAGE_KEY), old);
  await dlg().getByRole('button', { name: '知識チェックの一覧へ', exact: true }).click(); await dlg().locator('[data-action="resume"]').click(); await dlg().locator('[data-question="2"]').waitFor();
  assert.ok(await dlg().locator('.tk-flag input').isChecked()); await answerCurrent(); await dlg().getByRole('button', { name: '中断・一覧', exact: true }).click();
  // Every topic has a guide and a basic-to-detail sequence without duplicating questions.
  await dlg().locator('[data-tab="topics"]').click(); assert.equal(await dlg().locator('[data-topic]').count(), 7);
  ...
  for (const topic of TOPICS) {
    ...
    assert.ok(!(await dlg().locator('[data-question]').count())); await bounds();
  ...
  await dlg().locator('[data-topic="perfect"]').click(); const course = TOPIC_SETS.filter(s => s.topicId === 'perfect');
  await dlg().locator(`[data-topic-set="${course[0].key}"]`).click();
  for (const id of course[0].ids) { await dlg().locator(`[data-question="${QUESTIONS.find(q => q.id === id)!.number}"]`).waitFor(); await answerCurrent(); await dlg().locator('[data-action="next"]').click(); }
  await dlg().locator('.tk-result').waitFor(); await dlg().getByRole('button', { name: `次へ：${course[1].label}`, exact: true }).click();
  const nextNumber = QUESTIONS.find(q => q.id === course[1].ids[0])!.number;
  await dlg().locator(`[data-question="${nextNumber}"]`).waitFor(); await page.reload({ waitUntil: 'networkidle' });
  await dlg().locator('[data-action="resume"]').click(); await dlg().locator(`[data-question="${nextNumber}"]`).waitFor();
  ...
  for (const [width, height, label] of [[320, 640, 'small'], [768, 1024, 'tablet'], [1440, 1000, 'desktop']] as const) {
    await page.setViewportSize({ width, height }); await dlg().locator('[data-tab="path"]').click(); await bounds(); await page.screenshot({ path: `${out}/${label}-home.png` });
  }
  await page.setViewportSize({ width: 390, height: 844 });
  for (const n of [1, 13, 31, 43, 49, 55, 91]) {
    await dlg().locator(`[data-start="${n}"]`).click(); await dlg().locator(`[data-question="${n}"]`).waitFor();
    await bounds(); await page.screenshot({ path: `${out}/mobile-question-${n}.png` }); await answerCurrent(); await bounds();
    if (n === 55) await page.screenshot({ path: `${out}/mobile-feedback.png` });
    await dlg().getByRole('button', { name: '中断・一覧', exact: true }).click();
  }
  await page.evaluate(() => document.documentElement.classList.add('dark')); await bounds(); await page.screenshot({ path: `${out}/mobile-dark-home.png` });
  await dlg().locator('[data-tab="review"]').click(); assert.ok(await dlg().locator('[data-review-topic]').count() > 0); await bounds(); await page.screenshot({ path: `${out}/mobile-review.png` });
  const reviewTopic = await dlg().locator('[data-review-topic]').first().getAttribute('data-review-topic'); await dlg().locator('[data-review-topic]').first().click();
  const reviewQ = Number(await dlg().locator('[data-question]').getAttribute('data-question')); assert.ok(TOPICS.find(t => t.id === reviewTopic)!.numbers.includes(reviewQ));
  ...
  assert.equal(await page.locator('[data-ui="knowledge-dialog"]').count(), 0); assert.equal(await page.locator('#root').evaluate(el => (el as HTMLElement).inert), false); assert.deepEqual(errors, []);
