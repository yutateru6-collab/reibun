...
async function lesson1(page) {
  ...
  await openHopeMenuFromList(page,'hope-lesson1');
}
async function hopeTest1(page) {
  await page.goto(BASE, { waitUntil: 'networkidle' });
  await page.getByRole('button', { name: /基本例文.*マスター/s }).click();
  await openHopeMenuFromList(page,'hope-test1');
}
async function cardToBack(page) {
  ...
  await (await firstStudyCard(page)).click();
}
...
async function auditEngine(engine, browserType) {
  ...
  await run(engine, 'standard mini survives resume and stays on same card', async () => {
    ...
    await lesson1(page); await page.getByRole('button', { name: /例文を覚える/ }).click(); await cardToBack(page);
    const mini = page.getByRole('button', { name: /ミニ解説を見る/ }); await mini.waitFor(); const before = await counter(page);
    await simulateResume(context, page); await mini.click(); await page.getByRole('button',{name:'ミニ解説を閉じる',exact:true}).waitFor(); await page.waitForTimeout(800);
    assert(await counter(page) === before, `card changed: ${before} -> ${await counter(page)}`);
    assert(await page.getByRole('button',{name:'ミニ解説を閉じる',exact:true}).isVisible(), 'mini explanation did not remain open');
  ...
  await run(engine, 'Hope official Test1 No.7 shows mini explanation instead of source panel', async () => {
    ...
    await hopeTest1(page); await page.locator('[data-mode=cloze]').click();
    ...
    await card.click();
    assert((await page.locator('[data-ui=hope-card-main]').innerText()).replace(/[\[\]]/g, '').trim() === 'To live is to learn.', 'Original Test1 No.7 completed sentence changed');
  ...
  await run(engine, 'memorize mini survives resume', async () => {
    const context = await newContext(); const page = await context.newPage();
    await lesson1(page); await page.locator('[data-mode=learn]').click(); await page.getByRole('button',{name:'日本語から見る',exact:true}).click(); await page.getByRole('button',{name:'タップして英文を見る',exact:true}).click();
    const mini = page.getByRole('button', { name: /ミニ解説を見る/ }); await mini.waitFor(); const before=await counter(page);
    await simulateResume(context,page); await mini.click(); await page.getByRole('button',{name:'ミニ解説を閉じる',exact:true}).waitFor();
  ...
  await run(engine, 'favorite and yet buttons never flip or navigate', async () => {
    const context=await newContext(); const page=await context.newPage(); await lesson1(page); await page.locator('[data-mode=learn]').click(); await cardToBack(page);
    const mini=page.getByRole('button',{name:/ミニ解説を見る/}); await mini.waitFor(); const before=await counter(page);
  ...
  await run(engine, 'prev and next change exactly one card', async()=>{
    const context=await newContext(); const page=await context.newPage(); await lesson1(page); await page.locator('[data-mode=learn]').click();
  ...
  await run(engine, 'freshness check never reloads an active study session', async()=>{
    const context=await newContext(); const page=await context.newPage(); await lesson1(page); await page.locator('[data-mode=learn]').click(); await cardToBack(page); const mini=page.getByRole('button',{name:/ミニ解説を見る/}); await mini.waitFor();
